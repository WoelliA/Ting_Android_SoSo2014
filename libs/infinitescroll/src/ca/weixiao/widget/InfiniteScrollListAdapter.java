 ...
 */
public abstract class InfiniteScrollListAdapter<T> extends ArrayAdapter<T> implements OnScrollListener {
	...
	public InfiniteScrollListAdapter(Context context, int resource, ArrayList<T> items){
	...
	public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
		if (view instanceof InfiniteScrollListView) {
			...
			int count = getCount();
			if (loadingMode == LoadingMode.SCROLL_TO_BOTTOM && lastItem >= count && canScroll) {
				onScrollNext();
			}
		}
	}
